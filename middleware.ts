...
export default clerkMiddleware(async (auth, req) => {
  ...
  if (subdomain) {
    ...
    // For the root path on a subdomain, rewrite to the restaurant page
    if (pathname === "/") {
      return NextResponse.rewrite(
        new URL(`/restaurants/${subdomain}`, req.url)
      );
    }
  ...
  // On the root domain, handle protected routes
  if (isProtectedRoute(req)) {
    await auth.protect();
  }

  // Allow normal access for root domain
  return NextResponse.next();
});
